...
import {
  ...
  FileText, Video, LayoutTemplate, Hash, // ← أضفنا Hash لأيقونة TikTok Pixel
} from 'lucide-react';
...
export default function ManageSettings({ storeSlug }) {
  ...
  const [tiktokPixelId, setTiktokPixelId] = useState(''); // ← الحقل الجديد
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        setTiktokPixelId(data.tiktok_pixel_id || ''); // ← جلب TikTok Pixel
  ...
  // ── التحقق من صحة معرف TikTok Pixel ──
  const isValidTiktokId = (id) => /^[A-Za-z0-9_\-]+$/.test(id.trim()) || id.trim() === '';
  ...
  const handleSaveSettings = async (e) => {
    ...
    try {
      ...
      const cleanTiktokId = tiktokPixelId.trim(); // تنظيف المعرف
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          tiktok_pixel_id: cleanTiktokId, // ← حفظ TikTok Pixel
  ...
  return (
    <div style={{ maxWidth: '760px', margin: '0 auto', direction: 'rtl', fontFamily: theme.font.base }}>
      ...
      <form onSubmit={handleSaveSettings} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        {/* ─── إعدادات TikTok Pixel (جديد) ─── */}
        <div style={{ ...cardStyle, padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px', border: '2px solid #000000' }}>
          <SectionLabel icon={<Hash size={15} />} label="TikTok Pixel" />
          <p style={{ margin: 0, color: theme.colors.textSubtle, fontSize: '12px' }}>
            أدخل رقم TikTok Pixel ID لتتبع زوار متجرك وإعادة الاستهداف وإحصائيات الإعلانات.
            <br />
            <a href="https://ads.tiktok.com/business/creativecenter/pixel/pc/en" target="_blank" rel="noopener noreferrer" style={{ color: theme.colors.accent }}>
              كيف تحصل على TikTok Pixel ID؟ ←
            </a>
          </p>
          <div>
            <label style={labelStyle}>رقم TikTok Pixel ID</label>
            <input
              type="text"
              value={tiktokPixelId}
              onChange={e => setTiktokPixelId(e.target.value)}
              placeholder="مثال: C98A7B3D5E1F2G4H"
              style={{
                ...inputStyle,
                borderColor: tiktokPixelId && !isValidTiktokId(tiktokPixelId) ? '#ef4444' : undefined,
                backgroundColor: tiktokPixelId && !isValidTiktokId(tiktokPixelId) ? '#fef2f2' : undefined,
              }}
            />
            {tiktokPixelId && !isValidTiktokId(tiktokPixelId) && (
              <p style={{ margin: '4px 0 0', fontSize: '11px', color: '#ef4444' }}>
                ⚠️ المعرف يجب أن يحتوي على أحرف وأرقام و _ و - فقط (بدون مسافات)
              </p>
            )}
            <p style={{ margin: '4px 0 0', fontSize: '11px', color: '#94a3b8' }}>
              اتركه فارغاً إذا لم ترغب في تفعيل التتبع.
